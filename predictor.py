...
#                     self.logger.info(f"Columns of input DataFrame before cleaning: {input_df.columns.tolist()}")
#
#                     # Apply data cleaning steps (order matters and should match training)
#                     input_df = drop_irrelevant_and_high_missing_columns(input_df.copy(), drop_threshold=0.5)
#                     input_df = process_time_column(input_df.copy())
#                     input_df = drop_highly_correlated_features(input_df.copy(), correlation_threshold=0.85)
#
#                     # Select only the features that were used for training (before pipeline)
#                     if self.feature_names is not None:
#                         try:
#                             input_df = input_df[self.feature_names]
#                             self.logger.info(f"Shape of input DataFrame after feature selection: {input_df.shape}")
#                             self.logger.info(f"Columns of input DataFrame after feature selection: {input_df.columns.tolist()}")
#                         except KeyError as e:
#                             self.logger.error(f"KeyError: Missing features in input data: {e}")
#                             return jsonify({'error': f'Missing features in input data: {e}'}), 400
#                     else:
#                         self.logger.warning("Feature names not loaded, proceeding without explicit feature selection.")
#
#                     self.logger.info("Applying preprocessing pipeline...")
#                     X_new_transformed = self.pipeline.transform(input_df)
#
#                     self.logger.info(f"Shape of transformed data: {X_new_transformed.shape}")
#
#                     self.logger.info("Making prediction...")
...
#
#                 else:
#                     self.logger.error("Invalid JSON format: Expected a dictionary of feature values.")
